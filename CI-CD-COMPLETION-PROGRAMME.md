...
```mermaid
flowchart TD
  G0["G0: authority and preimages"] --> TX1["TX1: Workspace inventory adapter"]
  TX1 --> T2["TX2: control tools"]
  TX1 --> T3["TX3: private verifier"]
  T3 --> G1["G1: private behavior"]
  T2 --> T4A["TX4A: public overlap"]
  T4A --> T4B["TX4B: private canary"]
  G1 --> T4B
  T4B --> T4C["TX4C: public waves"]
  T4C --> T5["TX5: private waves and retirement"]
  T2 --> T6A["TX6A: public callers A"]
  T4C --> T6A
  T2 --> T6B["TX6B: private callers A"]
  T5 --> T6B["TX6B: private callers A"]
  T6A -. "conditional blockers" .-> TR["TX-R: owner repairs"]
  T6B -. "conditional blockers" .-> TR
  T6A --> T7["TX7: central bridge"]
  T6B --> T7
  TR --> T7
  T7 --> T8A["TX8A: public input removal"]
  T7 --> T8B["TX8B: private input removal"]
  T8A --> T9A["TX9A: Primitives wrapper"]
  T8A --> T9B["TX9B: Standards wrapper"]
  T8A --> T9C["TX9C: Foundations wrapper"]
  T8B --> T9A
  T8B --> T9B
  T8B --> T9C
  T5 --> T9A
  T5 --> T9B
  T5 --> T9C
  T9A --> T10["TX10: central cleanup"]
  T9B --> T10
  T9C --> T10
  T10 --> T11["TX11: Skills"]
  T10 --> T12["TX12: probe and deletion"]
  T2 --> TR
  T11 --> V1["V1: final sweep"]
  T12 --> V1
  T10 --> V1
  TR --> V1
  V1 --> E1["E1: external review"]
  E1 --> R1["R1: receipt and closure"]
```
