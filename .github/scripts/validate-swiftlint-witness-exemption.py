#!/usr/bin/env python3
"""validate-swiftlint-witness-exemption.py — verify the #219 protocol-witness
exemption in the canonical `typed_throws_required` / `no_any_protocol_existential`
custom SwiftLint rules.

swift-institute/.github#219 (principal ruling, comment 5164746902,
2026-08-03): the two custom rules' regexes carry a bounded-lookbehind /
lookaround exemption for stdlib `Encodable.encode(to:)` /
`Decodable.init(from:)` witnesses (both the `any Encoder`/`any Decoder` and
the pre-`any` spelling) and for SwiftSyntaxMacros `expansion(...)`
witnesses. The exemption must fire ONLY on those exact witness shapes —
ordinary untyped `throws` and ordinary `any <Protocol>` existentials, plus
near-miss shapes that merely resemble a witness (different internal
parameter name, or the right internal name typed against a different
protocol), MUST still be flagged.
...
NSRegularExpression regex behavior, which a Python reimplementation could
silently drift from — against every `*.swift` file under the fixture's
`Sources/` tree, and report any `typed_throws_required` /
`no_any_protocol_existential` finding.
...
scanning a fixture directory (which is not layout-checked out at the
config's own `Sources/`) silently reports zero lintable files. Explicit
file arguments bypass that `included:` resolution and are linted directly.
"""
...
RELEVANT_RULE_IDS = ("typed_throws_required", "no_any_protocol_existential")

# Single TSV prefix for both underlying SwiftLint rules: run.sh's harness
# only needs to know whether the #219 exemption predicate misbehaved on a
# given fixture, not which of the two rules fired. Individual per-file
# per-rule detail is included in the emitted message for debugging.
FINDING_PREFIX = "SWIFTLINT-WITNESS-EXEMPTION"


def canonical_config_path() -> Path:
    """Return <repo_root>/.swiftlint.yml for the checkout this script lives in.

    This script is at <repo_root>/.github/scripts/validate-swiftlint-witness-
    exemption.py; the canonical Tier 1 config under test is always the real
    <repo_root>/.swiftlint.yml, never a per-fixture file (the fixtures under
    test carry no `.swiftlint.yml` of their own — they exist only to be
    linted against the canonical one).
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        sys.exit("usage: validate-swiftlint-witness-exemption.py <owner/name> <repo_root>")
