...
extension Canon.Citation {
    ...
    public static func scan(_ line: String) -> [Self] {
        ...
        for position in brackets.indices.dropLast() where !consumed.contains(position) {
            ...
            guard Canon.Identifier.isNumeric(left.inner),
                Canon.Identifier.isNumeric(right.inner),
        ...
        for position in brackets.indices where !consumed.contains(position) {
            ...
            guard Canon.Identifier.isPrefix(family) else { continue }
            consumed.insert(position)
        ...
        for position in brackets.indices where !consumed.contains(position) {
            let inner = brackets[position].inner
            guard Canon.Identifier.isNumeric(inner) || Canon.Identifier.isWord(inner) else {
                continue
            }
    ...
    static func inBracketRange(_ inner: String) -> (Canon.Identifier, Canon.Identifier)? {
        ...
        where character == "-" || character == "–" || character == "—" {
            ...
            guard Canon.Identifier.isNumeric(head) else { continue }
            ...
            guard let family = Canon.Identifier(head).parts?.family else { continue }
