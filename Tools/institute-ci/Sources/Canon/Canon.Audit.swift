...
extension Canon.Audit {
    /// Every identifier the corpus defines, and where.
    public static func definitions(in corpus: Canon.Corpus) -> [Canon.Identifier: [Canon.Definition]]
    {
        var definitions: [Canon.Identifier: [Canon.Definition]] = [:]
        for document in corpus.documents {
            for line in document.prose {
                guard let identifier = Canon.Document.headingDefinition(on: line.text) else {
                    continue
                }
    ...
    /// carry all three.
    public static func registered(in document: Canon.Document) -> Set<Canon.Identifier> {
        var identifiers: Set<Canon.Identifier> = []
        for line in document.prose where Canon.Document.isRegistryHeader(line.text) {
            for citation in Canon.Citation.scan(line.text) {
                switch citation.form {
                case .crossRange(let start, let end), .inBracketRange(let start, let end):
                    identifiers.formUnion(Canon.Identifier.range(from: start, to: end))
...
extension Canon.Audit {
    ...
    func citations(against definitions: [Canon.Identifier: [Canon.Definition]]) -> [Canon.Finding] {
            ...
            for line in document.prose {
                ...
                for citation in Canon.Citation.scan(line.text) {
                    ...
                    case .single(let identifier):
                        ...
                        guard
                            !(Canon.Identifier.isWord(identifier.rawValue)
    ...
    static func deduplicated(_ findings: [Canon.Finding]) -> [Canon.Finding] {
        var seen: Set<String> = []
...
extension Canon.Audit {
    func duplicates(in definitions: [Canon.Identifier: [Canon.Definition]]) -> [Canon.Finding] {
        var findings: [Canon.Finding] = []
...
extension Canon.Audit {
    ...
    func resolves(_ token: String, citedBy document: Canon.Document) -> Bool {
        if token.hasPrefix("~") {
        ...
        for skill in corpus.skills {
            ...
            for document in skill.companions + [hub] {
                let body = document.prose
                    .filter { !Canon.Document.isRegistryHeader($0.text) }
                    .map(\.text)
                    .joined(separator: "\n")
