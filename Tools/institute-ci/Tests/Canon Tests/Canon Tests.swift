...
extension Canon {
    @Suite
    struct Tests {
        static func corpus(_ files: [(alias: String, text: String)]) -> Canon.Corpus {
            .init(
                documents: files.map {
        ...
        static func audit(_ corpus: Canon.Corpus) -> Canon.Audit {
            .init(
        ...
        struct Unit {
            @Test func `a numeric identifier splits at its last number group`() throws {
                let parts = try #require(Canon.Identifier("API-NAME-001").parts)
            ...
            @Test func `a letter suffix is part of the identifier, not of the number`() throws {
                let parts = try #require(Canon.Identifier("PLAT-ARCH-008g").parts)
            ...
            @Test func `a word-form identifier has no number to compare`() {
                #expect(Canon.Identifier("IMPL-INTENT").parts == nil)
                #expect(Canon.Identifier.isWord("IMPL-INTENT"))
                #expect(!Canon.Identifier.isWord("API-NAME-001"))
                #expect(Canon.Identifier.isNumeric("API-NAME-001"))
            }
            ...
            @Test func `ordering is numeric within a family`() {
                #expect(Canon.Identifier("X-9") < Canon.Identifier("X-10"))
            }

            @Test func `a range expands to its members at the start's width`() {
                #expect(
                    Canon.Identifier.range(from: "API-NAME-009", to: "API-NAME-011")
                        == ["API-NAME-009", "API-NAME-010", "API-NAME-011"])
            }

            @Test func `a single cite is recognised in both grammars`() {
                #expect(Canon.Citation.scan("see [API-NAME-001]").count == 1)
                #expect(Canon.Citation.scan("see [IMPL-INTENT]").count == 1)
                #expect(Canon.Citation.scan("see [lowercase-001]").isEmpty)
            }
            ...
            @Test func `a wildcard names its family`() {
                guard case .wildcard(let family) =
                    Canon.Citation.scan("the [MEM-COPY-*] family").first?.form
                else {
            ...
            func `an anchored path is a verifiable citation`(_ token: String) {
                #expect(Canon.Audit.looksLikePath(token))
                #expect(Canon.Audit.isAnchored(token))
            }
            ...
            @Test func `the census recognises both definition forms`() {
                #expect(Canon.Census.isHeadingRow("### [API-NAME-001]"))
                #expect(Canon.Census.isHeadingRow("## [API-NAME-001] Title"))
                #expect(!Canon.Census.isHeadingRow("# [API-NAME-001]"))
                #expect(Canon.Census.isTableRow("| [API-NAME-001] | statement |"))
                #expect(!Canon.Census.isTableRow("| API-NAME-001 | statement |"))
            }
            ...
            @Test func `the baseline key carries no line number`() {
                let finding = Canon.Finding(check: .citations, key: "a.md [A-002]", message: "a.md:9 …")
        ...
        struct Integration {
            @Test func `a dangling citation is reported and a resolved one is not`() {
                let corpus = Canon.Tests.corpus([("a.md", "### [A-001]\nSee [A-001] and [A-002].\n")])
                let findings = Canon.Tests.audit(corpus).run([.citations]).results[.citations] ?? []
            ...
            @Test func `a historical marker exempts the whole line`() {
                let corpus = Canon.Tests.corpus([
                    ("a.md", "### [A-001]\n[A-002] was retired; the slot stays burned.\n")
                ])
                #expect(Canon.Tests.audit(corpus).run([.citations]).results[.citations]?.isEmpty == true)
            }

            @Test func `template grammar is not a citation`() {
                let corpus = Canon.Tests.corpus([("a.md", "Use [PREFIX-001] or [FOO-NNN].\n")])
                #expect(Canon.Tests.audit(corpus).run([.citations]).results[.citations]?.isEmpty == true)
            }
            ...
            @Test func `two heading sites for one identifier is a duplicate`() {
                ...
                let findings = Canon.Tests.audit(corpus).run([.duplicates]).results[.duplicates] ?? []
                #expect(findings.count == 1)
            ...
            @Test func `a registry entry beside a heading is not a duplicate`() {
                ...
                #expect(
                    Canon.Tests.audit(corpus).run([.duplicates]).results[.duplicates]?.isEmpty == true)
            }
            ...
            @Test func `a baselined finding is not counted as new`() {
                let corpus = Canon.Tests.corpus([("a.md", "See [A-002].\n")])
                let audit = Canon.Audit(
            ...
            @Test func `the report renders the retired engine's lines`() {
                let corpus = Canon.Tests.corpus([("a.md", "See [A-002].\n")])
                let lines = Canon.Tests.audit(corpus).run([.citations]).lines(enforcing: false)
        ...
        struct `Edge Case` {
            ...
            /// phantom definitions.
            @Test(arguments: [
                (Canon.Identifier("A-001"), Canon.Identifier("B-003")),
                ("A-001a", "A-003"),
                ("A-005", "A-001"),
                ("A-001", "A-900"),
                ("IMPL-INTENT", "A-003"),
            ...
            func `an unexpandable range keeps only its endpoints`(
                _ start: Canon.Identifier, _ end: Canon.Identifier
            ) {
                #expect(Canon.Identifier.range(from: start, to: end) == [start, end])
            }
            ...
            @Test func `frontmatter is not scanned`() {
                let corpus = Canon.Tests.corpus([("a.md", "---\nhistory: dropped [A-002]\n---\nBody.\n")])
                #expect(Canon.Tests.audit(corpus).run([.citations]).results[.citations]?.isEmpty == true)
            }
            ...
            @Test func `an unanchored relative path is not verifiable`() {
                #expect(Canon.Audit.looksLikePath(".github/workflows/ci.yml"))
                #expect(!Canon.Audit.isAnchored(".github/workflows/ci.yml"))
            }

            /// No roots is no measurement, not a count of zero.
            @Test func `an empty root set is a defect`() {
                #expect(throws: Canon.Census.Error.noRoots) {
                    _ = try Canon.Census.taken(over: [])
                }
            }
        }
    }
